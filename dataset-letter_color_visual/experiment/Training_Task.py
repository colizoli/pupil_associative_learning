...
### PARAMETERS ###
# Timing in seconds
REPS        = 10        # how many times to repeat all trials in training_balancing
...
training_trials = pd.read_csv(os.path.join('stimuli','training_balancing.csv'))
...
if subject_ID:
    ...
    stim_sq     = visual.Rect(win, width=p.sqw, height=p.sqh, pos=(0.0, 0.0))
    ...
    if debug_mode:
        ALL_TRIALS = ALL_TRIALS.iloc[0:25,:] # just get first 25 trials
    ...
    for t in range(len(ALL_TRIALS)):
        ...
        elif this_color_idx == 100: # random normal color
