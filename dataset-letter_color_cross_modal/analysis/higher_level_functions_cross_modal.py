...
class higherLevel(object):
    ...
    def create_subjects_dataframe(self,blocks):
        ...
        # flags outlier RTs and missing trials from concantenated dataframe
        ...
        for f in files:
            ...
            ###############################            
            this_data['missing'] = this_data['button']=='missing' # based on response
            # check if pupil data missing (can happen on last trial in later time window)
            this_data['missing_pupil'] = pd.isna(this_data[this_data.columns[this_data.columns.str.contains('pupil')]]).sum(axis=1)
            # combine
            this_data['drop_trial'] = np.array(this_data['missing']) | np.array(this_data['missing_pupil'])#logical or
        ...
        print('Missing Pupil = {}%'.format(np.true_divide(np.sum(DF['missing_pupil']),DF.shape[0])*100))
    ...
    def average_conditions(self):
        ...
        for pupil_dv in ['pupil_resp_locked_t1','pupil_resp_locked_t2','RT']: #interaction accuracy and frequency
        
            ###########
            # REGRESS RT OUT OF RESPONSE-LOCKED PUPIL, PER SUBJECT
            # RT log-transformed and z-scored  
            if 'resp' in pupil_dv:
                save_residuals = []
                for s, subj in enumerate(np.unique(DF['subject'])): # REGRESS RT OUT OF RESPONSE-LOCKED PUPIL DILATION
                    this_subj = DF[DF['subject']==subj].copy()
                    RT = stats.zscore(np.log(np.array(this_subj['RT'])))
                    Y = np.array(this_subj[pupil_dv])
                    X = np.array([RT]) # have to make X a column vector
                    YRES = self.lin_regress_residuals(Y, X, eq='ols')
                    save_residuals.append(YRES)
                DF[pupil_dv] = np.concatenate(save_residuals) # replace column with RT-regressed phasic pupil responses
    ...
    def plot_phasic_pupil_pe(self,):
        ...
        dvs = ['pupil_resp_locked_t1','pupil_resp_locked_t2']
    ...
    def dataframe_evoked_pupil_higher(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            # get averages for different conditions
            for c,cond in enumerate(csv_names):
                ...
                for s,subj in enumerate(self.subjects):
                    ...
                    # #############################
                    # #### REGRESS RT OUT OF EACH TIME POINT IN THE KERNEL ####
                    # # RT log-transformed and z-scored
                    # RT = stats.zscore(np.log(np.array(SDATA['RT'])))
                    # X = np.array([RT]) # have to make X a column vector
                    # # loop time points
                    # for tp in evoked_cols:
                    #     Y = np.array(SDATA[tp])
                    #     YRES = self.lin_regress_residuals(Y, X, eq='ols')
                    #     SDATA[tp] = YRES # replace column with residuals
                    # #############################
