...
class GeneralLinearModel(object):
    ...
    def configure(self, IRF='pupil', IRF_params=None, regressor_types=['stick'], demean=False, basis_set=False, normalize_sustained=False):
        ...
        for i, reg in enumerate(self.event_object):
            if regressor_types[i] == 'stick':
                self.add_stick_regressor(np.atleast_2d(reg))  # adjusts the raw_design_matrix
        ...
        if IRF == 'pupil':
            ...
            self.IRF = IRF # blink and sac kernels
    ...
    def convolve_with_IRF(self):
        ...
        # does all combinations: if 2 events, 2 IRFs:
        # event1-IRF1, event1-IFR2, event2-IRF1, event2-IRF2|
        # only the first and last are of interest
        # print
        # print len(self.IRF)
