...
# encoding: utf-8
"""
Analysis gradient prediction errors - Feedback control task, mean responses and higher level analyses
O.Colizoli 2019
"""
...
class higherLevel(object):
    def __init__(self, subjects, experiment_name, project_directory, sample_rate, time_locked, pupil_step_lim, baseline_window, pupil_time_of_interest):        
        ...
        ##############################
        self.factors = [
            ['tone'], # rgb separate columns, so just use one column for individual colors
        ] 
        self.csv_names = [
            'tone',
        ]
    ...
    def plot_phasic_pupil(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            for pupil in pupil_dvs:
                ...
                xind = np.array([0.3,0.35])
                xlim = [0.25,0.4]
                bar_width = 0.05
                colors = ['red','green']
                # plot bar graph
                for x in [0,1]:
                    ax.bar(xind[x],np.array(MEANS['mean'][x]), width=bar_width, yerr=np.array(MEANS['sem'][x]), color=colors[x], alpha=1, edgecolor='white', ecolor='black')
    ...
    def plot_evoked_pupil(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            # stats
            pupil_control.cluster_sig_bar_1samp(array=save_conds[0]-save_conds[1], x=pd.Series(range(TS.shape[-1])), yloc=1, color='black', ax=ax, threshold=0.05, nrand=5000, cluster_correct=True)
