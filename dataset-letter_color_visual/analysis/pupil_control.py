...
# encoding: utf-8
"""
Control experiment for Letter-color associations formed through statistical learning in the visual modality
...
class pupilPreprocess(object):
    """pupilPreprocessing"""
    def __init__(self, subject, edf, experiment_name, project_directory, eye, msgs, tolkens, sample_rate, tw_blinks):
    ...
    def convert_edfs(self,):
        ...
        # NOTE calling executable STOPPED WORKING ON LISA MAY 2020, USED CONVERTER APP EYELINK EDF > ASC      
        # terminal commands
 ...
 #                        shell()
    ...
    def percent_signal_change(self,):
        ...
        if self.add_base: # did not regress out blinks/saccades
            self.pupil = self.pupil + self.pupil_baseline.mean()
...
class trials(object):
    ...
    def event_related_subjects(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            for s,subj in enumerate(self.subjects):
                ...
                r = len(phase_idx)
                c = int((self.pupil_step_lim[1]-self.pupil_step_lim[0])*self.sample_rate)
                ...
                for trial,t_idx in enumerate(phase_idx):
                    ...
                    this_pupil = pupil[int(t_idx+(self.pupil_step_lim[0]*self.sample_rate)):int(t_idx+(self.pupil_step_lim[1]*self.sample_rate))]                    
    ...
    def plot_event_related(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            ax.axvline(int(abs(self.pupil_step_lim[0]*self.downsample_rate)), lw=0.25, alpha=0.5, color = 'k') # Add vertical line at t=0
            ax.axhline(0, lw=0.25, alpha=0.5, color = 'k') # Add horizontal line at t=0
            
            # determine time points x-axis given sample rate
            event_onset = int(abs(self.pupil_step_lim[0]*self.downsample_rate))
            end_sample = int((self.pupil_step_lim[1] - self.pupil_step_lim[0])*self.downsample_rate)
            ...
            tw_begin = int(event_onset + (self.pupil_time_of_interest[0]*self.downsample_rate))
            tw_end = int(event_onset + (self.pupil_time_of_interest[1]*self.downsample_rate))
        ...
        print('sucess: plot_event_related_time_window')
    ...
    def plot_event_related_subjects(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            for s,subj in enumerate(self.subjects):
                ...
                ax = fig.add_subplot(8,6,s+1)
                
                # Downsample pupil data for plotting
                M = sp.signal.decimate(np.array(M), int(self.downsample_factor), ftype='fir')
                # plot 
                # sns.tsplot(M, condition=time_locked, value='Pupil response\n(% signal change)', color='black', alpha=1, lw=1, ls='-', ax=ax)
        ...
        print('sucess: plot_event_related_time_window_subjects')
    ...
    def phasic_pupil(self):
        ...
        for t,time_locked in enumerate(self.time_locked):
            for s,subj in enumerate(self.subjects):
                ...
                SAVE_TRIALS = []
                for trial in range(len(P)):
                    phase_start = int((self.baseline_window*self.sample_rate) + (self.pupil_time_of_interest[0]*self.sample_rate))
                    phase_end = int((self.baseline_window*self.sample_rate) + (self.pupil_time_of_interest[1]*self.sample_rate))
                ...
                B.drop(['Unnamed: 0'],axis=1,inplace=True)
                B.to_csv(behav_file)
                print('subject {}, {} phasic pupil extracted {}'.format(subj,time_locked,self.pupil_time_of_interest))
        print('sucess: phasic_pupil')