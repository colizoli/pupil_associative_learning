...
# early time window
(hist_t1 <- ggplot(df, aes(x = pupil_feed_locked_t1)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t1") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    

# late time window
(hist_t2 <- ggplot(df, aes(x = pupil_feed_locked_t2)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t2") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    
...
if (run_models) {
  ...
  model1_t1 <- brms::brm(pupil_feed_locked_t1 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
  ...
  # fixed slope with random intercept
  model2_t1 <- brms::brm(pupil_feed_locked_t1 ~ model_i + model_H*model_D + pupil_baseline_feed_locked + RT + (1|subject),
                         data = df, family = gaussian(), chains = nchains,
                         iter = iters, warmup = warmups)
  saveRDS(model1_t1, "linear_mixed_models/model1_t1") # to save and prevent running again later
...
if (run_models) {
  # fixed slope with random intercept
  model1_t2 <- brms::brm(pupil_feed_locked_t2 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
                         data = df, family = gaussian(), chains = nchains,
                         iter = iters, warmup = warmups)
  saveRDS(model1_t2, "linear_mixed_models/model1_t2") # to save and prevent running again later
  
  # fixed slope with random intercept
  model2_t2 <- brms::brm(pupil_feed_locked_t2 ~ model_i + model_H*model_D + pupil_baseline_feed_locked + RT + (1|subject),
                         data = df, family = gaussian(), chains = nchains,
                         iter = iters, warmup = warmups)
...
# check whether the DVs are normally distributed
# early time window
(hist_t1 <- ggplot(df_correct, aes(x = pupil_feed_locked_t1)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t1 correct") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    

# late time window
(hist_t2 <- ggplot(df_correct, aes(x = pupil_feed_locked_t2)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t2 correct") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    

(hist_t1 <- ggplot(df_error, aes(x = pupil_feed_locked_t1)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t1 error") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    

# late time window
(hist_t2 <- ggplot(df_error, aes(x = pupil_feed_locked_t2)) +
    geom_histogram(colour = "#8B5A00", fill = "#CD8500") +
    theme_bw() +
    ylab("Count\n") +
    xlab("\npupil_feed_locked_t2 error") +  # latin name for red knot
    theme(axis.text = element_text(size = 12),
          axis.title = element_text(size = 14, face = "plain")))    
...
if (run_models) {
  # fixed slope with random intercept
  model1_t1_correct <- brms::brm(pupil_feed_locked_t1 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
  ...
  # fixed slope with random intercept
  model2_t1_correct <- brms::brm(pupil_feed_locked_t1 ~ model_i + model_H*model_D + pupil_baseline_feed_locked + RT + (1|subject),
                                 data = df_correct, family = gaussian(), chains = nchains,
                                 iter = iters, warmup = warmups)
...
if (run_models) {
  ...
  # fixed slope with random intercept
  model1_t2_correct <- brms::brm(pupil_feed_locked_t2 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
                                 data = df_correct, family = gaussian(), chains = nchains,
                                 iter = iters, warmup = warmups)
  saveRDS(model1_t2_correct, "linear_mixed_models/model1_t2_correct") # to save and prevent running again later
  
  # fixed slope with random intercept
  model2_t2_correct <- brms::brm(pupil_feed_locked_t2 ~ model_i + model_H*model_D + pupil_baseline_feed_locked + RT + (1|subject),
                                 data = df_correct, family = gaussian(), chains = nchains,
                                 iter = iters, warmup = warmups)
...
if (run_models) {
  ...
  # fixed slope with random intercept
  model1_t1_error <- brms::brm(pupil_feed_locked_t1 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
  ...
  # fixed slope with random intercept
  model2_t1_error <- brms::brm(pupil_feed_locked_t1 ~ model_i + pupil_baseline_feed_locked + RT + model_H*model_D + (1|subject),
                               data = df_error, family = gaussian(), chains = nchains,
                               iter = iters, warmup = warmups)
...
if (run_models) {
  ...
  # fixed slope with random intercept
  model1_t2_error <- brms::brm(pupil_feed_locked_t2 ~ model_i + model_H + model_D + pupil_baseline_feed_locked + RT + (1|subject),
                               data = df_error, family = gaussian(), chains = nchains,
                               iter = iters, warmup = warmups)
  saveRDS(model1_t2_error, "linear_mixed_models/model1_t2_error") # to save and prevent running again later
  
  # fixed slope with random intercept
  model2_t2_error <- brms::brm(pupil_feed_locked_t2 ~ model_i + pupil_baseline_feed_locked + RT + model_H*model_D + (1|subject),
                               data = df_error, family = gaussian(), chains = nchains,
                               iter = iters, warmup = warmups)
